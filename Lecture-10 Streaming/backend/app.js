...
app.get("/highmemory",function(req,res){

    // the whole file will go to the RAM 
    const file = fs.readFileSync("Minor-project.zip");
    res.send(file);
})

app.get("/stram",function(req,res){

    // the file will go now in chunck
     
    // request - read stream 
    // response - write stream 
    const fileReadStream = fs.createReadStream("Minor-project.zip");
...
app.get("/videostream",function(req,res){
    ...
    // it will send the video in chunck 
    const videoStreamInstance = fs.createReadStream("music.mp4");
    videoStreamInstance.pipe(res); 
})


app.get("/videostreamrange",function(req,res){
    ...
    if (range) {
        ...
        const stat = fs.statSync("music.mp4");
        ...
        // streaming the video 
        const videoStreamInstance = fs.createReadStream("music.mp4",{start,end});
        videoStreamInstance.pipe(res); 
    }else{
        res.status(400).json({
